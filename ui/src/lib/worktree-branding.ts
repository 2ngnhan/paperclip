...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("nclerk-worktree-enabled") !== "true") return null;

  const name = readMetaContent("nclerk-worktree-name");
  const color = normalizeHexColor(readMetaContent("nclerk-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("nclerk-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
